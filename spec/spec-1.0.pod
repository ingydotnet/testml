...
- Make testml-pm match the current spec.
- Use testml-pm in yaml-perl
...
=item head-section := ( meta-statement | test-statement | include-statements | comment | blank-line )+
...
=item test-statement := data-expression space+ operator space+ data-expression eol
...
=item point-name := [A-Za-z][A-Za-z0-9_]*
...
=item transform-name := [A-Za-z][A-Za-z0-9_]*
...
=item include-statements := include-statement ( include-statement | comment | blank-line )*

A group of adjacent include statements is processed at once. All of the
...
the data section, where '%include' statements are no longer recognized.
...
The included file must end with a newline, unless the file is empty.
...
Setting this option changes the parser to look for the specified pattern
to find the start of the data section.
...
Files are included by a relative path. If the file does not exist, and
thus can not be included, it must be considered an error.
