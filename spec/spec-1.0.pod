...
not used for any purpose in the TestML document object. In other words,
they are thrown out.
...
=item meta-keyword :=
    ( 'title' | 'tests' | 'data' | 'testml_block_marker' | 'testml_point_marker') |
    ( 'x_' WORD+ )

The listed keywords represent the meta settings supported in TestML 1.0.
See L<Valid Meta Keywords> for the definitions of them.

A document also allows for user defined meta keywords that begin
with 'x_'.
...
=item test-section := ( test-statement | ws )* (comment | EOL)
...
The parsing of the meta section should instantiate a Test object.
...
=item data-expression := data-sub-expression ( call-indicator data-subexpression )*

A data expression is a series of one or more subexpressions which are
evaluated in order, passing the result of each to the next one.

=item test-assertion := data-expression comparison-expression
...
=item data-sub-expression := point-name | transform-call

A sub expression can either be a plain point name or a transform
function call.
...
=item comparison-expression := ws+ operator ws+ data-expression

A comparison expression is an operator followed by a data expression
that represents the expected value. The operator must have some
whitespace on either side.

=item point-name := LOWER WORD*

A data point name is an identifier that begins with a lower case letter.
...
The only defined operator currently is C<==>.

=item transform-name := core-transform-name | user-transform-name

A transform is either a standard core one, or a user defined transform.

=item core-transform-name := UPPER WORD*

A core data transform is defined in the TestML Standard Library.

=item user-transform-name := LOWER WORD*

A user data transform name is an identifier that begins with a lower case
...
=item argument := data-expression | quoted-string
...
=item lines-point := point-marker SPACE+ point-name SPACE* EOL (line - block-header)*
...
=item phrase-point := point-marker SPACE+ point-name ':' (SPACE NON-BREAK*)? EOL blank-line*
...
implementation may complain or raise an exception.
...
The file extension is used to determine the format of the data section encoded
in the file.
...
A file extension can be added to the inline pseudoname, like '_.xml', but this
