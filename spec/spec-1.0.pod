...
=item point-name := user-point-name | core-point-name

A data point name is either a user defined data point, or a core global
data point.

=item user-point-name := LOWER WORD*

A user data point name begins with a lower case letter. It refers to an
element of the current data block.

=item core-point-name := UPPER WORD*
...
=item transform-name := user-transform-name | core-transform-name
...
=item test-assertion := data-expression assertion-expression
...
=item assertion-expression := inline-assertion | function-assertion
...
=item inline-assertion-expression := ws+ assertion-operator ws+ data-expression
...
=item function-assertion :=
    call-indicator assertion-name '(' ws* data-expression ws* ')'
