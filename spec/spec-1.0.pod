...
of which make assertions. The following statements are valid:
    ...
    full_expression1 == full_expression2;

The first five statements are test expressions. The last two lines are
test assertions which test the results of two expressions. The two are
equivalent operations using different syntax.
...
=item sub-expression := (transform-call | data-point)
...
except that the bareword point names are also used to select the data
